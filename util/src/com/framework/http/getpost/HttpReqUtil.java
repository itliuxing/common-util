...
//			httpGet.setConfig(requestConfig); // 父类定义的超时信息，在每个请求都要置入配置参数
	...
	public static void main(String[] args) {
		...
		try {
			...
			//测试商城处理
			MallNotifyEntity notify = new MallNotifyEntity() ;
			notify.setAppid("zhdsbang");
			notify.setVersion("1.0") ;
			notify.setOrgid("test_org1234567") ;
			notify.setShanghui_name("测试数据-测试商会名称-1") ;
			notify.setAddress("测试数据-测试商会地址-1") ;
			//httpResponse = HttpReqUtil.getObjectReq("http://api.shangbangmall.com/shanghui/modify", notify ) ;
			//httpResponse = HttpReqUtil.postObjectReqForm("http://api.shangbangmall.com/shanghui/modify", notify ) ;
			httpResponse = HttpReqUtil.	postSendReqForm("http://api.shangbangmall.com/shanghui/modify", notify ) ;
