 ...
 * 类描述：   		调用Google二维码生成器
 * 创建人：		
 * 创建时间：		2016-10-27上午9:53:36 
 * 修改人：		liuxing
 * 修改时间：		2016-10-27上午9:53:36 
 * 修改备注：   		jar：core-3.1.0.jar
 * maven		<dependency>
...
public class QrCodeUtil {
	...
	public static void main(String[] args) throws WriterException {
		try {
			//getLogoQRCode("https://www.baidu.com/", "跳转到百度的二维码");		//高级生成器---二维码下面还生成一段文字，不指定生成文件
			
			createQRCode("https://www.baidu.com/", "C:/Users/liuxing/Desktop/TDC-123.jpg");		//普通生成器
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/****
	 * 生成二维码
	 * @param qrUrl			指定链接或者文字为二维码的信息
	 * @param filePath		生成二维码的文件地址
	 * @param fileName		生成二维码的文件名称
	 */
	public static boolean createQRCode(String qrUrl, String filePath ) {
		return createQRCode(qrUrl, filePath , 400 , 400 ) ;
	}
	
	/****
	 * 生成二维码
	 * @param qrUrl			指定链接或者文字为二维码的信息
	 * @param filePath		生成二维码的文件保存
	 * @param fileName		生成二维码的文件名称
	 * @param width			二维码宽度
	 * @param height		二维码高度
	 * @return
	 */
	public static boolean createQRCode(String qrUrl, String filePath , int width , int height ) {
		try {
			BufferedImage bufImg = QrCodeUtil.getQR_CODEBufferedImage( qrUrl,
					BarcodeFormat.QR_CODE, width , height , QrCodeUtil.getDecodeHintType() );		//生成
			WriteQRCodeNative( bufImg , filePath ) ;		//写入本地
			return true ;
		} catch (Exception e) {
			//e.printStackTrace();
			return false ;
	...
	public static String getLogoQRCode(String qrUrl, String productName) {
		...
		String filePath = "C:/Users/liuxing/Desktop/20161027094952.png"; // TODO
		String content = qrUrl;
		try {
			BufferedImage bim = QrCodeUtil.getQR_CODEBufferedImage(content,
					BarcodeFormat.QR_CODE, 400, 400, QrCodeUtil.getDecodeHintType() );
			return QrCodeUtil.addLogo_QRCode(bim, new File(filePath), new LogoConfig(),
					productName);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param logoPic
	 */
	public static void WriteQRCodeNative( BufferedImage bim , String filePath )throws Exception {
			/**
			 * 读取二维码图片，并构建绘图对象
			 */
	 ...
	 * @param logoPic
	 */
	public static String addLogo_QRCode(BufferedImage bim, File logoPic,
		...
		try {
			...
			ImageIO.write(image, "png", new File(
					"C:/Users/liuxing/Desktop/TDC-" + new Date().getTime()
							+ "test.png")); // TODO
